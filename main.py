...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Set up a timer to read the webcam every 30 milliseconds
        self.timer = QTimer()
        self.timer.timeout.connect(self.update_frame)
        self.timer.start(30)

        # Set the window properties
        self.setWindowTitle("Face Mask Recognition")
    ...
    def update_frame(self):
        ...
        # Perform face mask prediction on the frame
        prediction_image = face_mask_prediction(frame)

        # Convert the prediction image to a QImage
        height, width, channels = prediction_image.shape
        bytes_per_line = channels * width
        q_image = QImage(prediction_image.data, width, height, bytes_per_line, QImage.Format_RGB888)
